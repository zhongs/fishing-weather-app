...
import { Fish, Search, Map as MapIcon } from 'lucide-react';
...
function App() {
  ...
  // 分析预报数据的钓鱼条件（简化版，用于5天预报）
  const analyzeForecastDay = (dayData) => {
    if (!dayData) return null;

    const temp = dayData.temp;
    const windSpeed = dayData.windSpeed;
    const weatherCode = dayData.weatherCode;
    const weatherCondition = getWeatherCondition(weatherCode).toLowerCase();
  ...
  const getWeatherIcon = (condition, size = 'w-16 h-16') => {
    ...
    if (main.includes('rain')) return <CloudRain className={size} />;
    if (main.includes('cloud')) return <Cloud className={size} />;
    if (main.includes('clear')) return <Sun className={size} />;
    if (main.includes('snow')) return <CloudSnow className={size} />;
            ...
            {forecast && forecast.length > 0 && (
              <div className="bg-white rounded-2xl shadow-2xl p-6">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h3 className="text-xl font-bold text-gray-800">未来5天预报</h3>
                </div>
                ...
                <div className="space-y-3">
                  {forecast.map((day, index) => {
                    ...
                    return (
                      <div 
                        key={index} 
                        className={`${analysis.bgColor} border border-gray-200 rounded-xl p-4 transition-all hover:shadow-md`}
                      >
                          ...
                          <div className="flex items-center gap-4 flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="text-gray-600">
                                {getWeatherIcon(getWeatherCondition(day.weatherCode), 'w-8 h-8')}
                              </div>
                              <div className="text-xs text-gray-600 max-w-[60px]">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-sm font-semibold text-gray-700">{day.tempMin}°</span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-xs text-gray-600">{day.windSpeed.toFixed(1)}m/s</span>
                            </div>
                          </div>
                          
                          {/* Fishing Score */}
                          <div className="text-right">
                            <div className={`${analysis.color} text-2xl font-bold`}>
                              {analysis.score}
                            </div>
                            <div className={`${analysis.textColor} text-xs font-medium`}>
                              {analysis.level}
                            </div>
                          </div>
                        </div>
